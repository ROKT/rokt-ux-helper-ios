...
@available(iOS 15.0, *)
final class TestOpacityModifier: XCTestCase {

    func test_column_with_opacity() throws {
        ...
        let opacityModifier = try hstack.modifier(OpacityModifier.self).actualView()
        XCTAssertEqual(opacityModifier.opacity, 0.5)
    }
    
    func get_model() throws -> ColumnViewModel {
        let transformer = LayoutTransformer(layoutPlugin: get_mock_layout_plugin())
        let column = ModelTestData.ColumnData.columnWithOpacity()
