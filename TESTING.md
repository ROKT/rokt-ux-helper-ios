...
Full-bleed lifecycle tests invoke the captured production completion and size callbacks. A resolver spy checks that completion replaces the previous height resolver before `onLoad`; it does not assert physical height. Dynamic sheets must wait for their first measurement, load once, and never register the percentage expansion observer.
...
For app-hosted restoration checks, supply the global expansion value before presentation and include a delayed presentation, so the initial state publication can precede the presentation controller. Assert that the actual frame matches the latest state when `onLoad` runs, then exercise subsequent expand/collapse actions. The opening animation still needs visual review; a correct final frame does not establish its first-frame appearance.
